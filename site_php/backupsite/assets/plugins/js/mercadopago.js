...
MercadoPago.prototype.setUrl = function (data) {
    this.url = data;
}
MercadoPago.prototype.saveSettings = function (form) {
    $.ajax({
        type: "post",
        dataType: "json",
...
MercadoPago.prototype.addPackage = function () {
    $.ajax({
        type: "post",
        dataType: "json",
        ...
        success: function (data) {
            if (data.error) {
                ...
                html += '<td class="center" style="display:none;"><input class="input-small" type="text" id="pack_country_' + data.id + '" value="' + $("#country_new").val() + '" /></td>';
                html += '</select></td>';
                html += '<td class="center" id="status_icon_' + data.id + '"><span class="label label-success">Active</span></td><td class="center"><a class="btn btn-danger" href="#" id="status_button_' + data.id + '" onclick="MercadoPago.changeStatus(' + data.id + ', 0);"><i class="icon-edit icon-white"></i> Disable</a>  <a class="btn btn-info" href="#" onclick="MercadoPago.edit(' + data.id + ');"><i class="icon-edit icon-white"></i> Edit</a>  <a class="btn btn-danger" href="#" onclick="MercadoPago.delete(' + data.id + ');"><i class="icon-trash icon-white"></i> Delete</a></td></tr>';
                $('#donate_sortable_mercadopago > tbody:last').append(html);
            }
        }
    });
}
MercadoPago.prototype.edit = function (id) {
    $.ajax({
        type: "post",
        dataType: "json",
...
MercadoPago.prototype.delete = function (id) {
    $.ajax({
        type: "post",
        dataType: "json",
...
MercadoPago.prototype.changeStatus = function (id, status) {
    $.ajax({
        type: "post",
        dataType: "json",
        ...
        success: function (data) {
            if (data.error) {
                ...
                if (status == 1) {
                    ...
                    $('#status_button_' + id).attr('onclick', 'MercadoPago.changeStatus(' + id + ', 0);');
                }
                else {
                    ...
                    $('#status_button_' + id).attr('onclick', 'MercadoPago.changeStatus(' + id + ', 1);');
                }
            }
        }
    });
}
MercadoPago.prototype.saveOrder = function () {
...
MercadoPago.prototype.checkout = function (id) {
    $.ajax({
