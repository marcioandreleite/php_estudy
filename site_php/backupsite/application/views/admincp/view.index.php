<div id="content" class="span10">
    ...
    <?php
        if(!file_exists(BASEDIR . 'setup' . DS . 'data' . DS . 'install.lock')){
            echo '<div class="alert alert-error span11">Please secure setup by creating file install.lock in '.BASEDIR . 'setup' . DS . 'data' . DS . '</div>';
        }
        if(!empty($tickets)){
            echo '<div class="alert alert-success span11">You have ' . count($tickets) . ' new support requests. <a href="' . $this->config->base_url . 'admincp/support-requests">Click To View</a></div>';
        }
        ...
        if($cron == false){
            echo '<div class="alert alert-error span11">Your scheduler is not working. please check your <a href="' . $this->config->base_url . 'admincp/manage-settings/scheduler" target="_blank">settings</a> and fix it.</div>';
        } else{
            if(time() - $cron > 720 * 2){
                echo '<div class="alert alert-error span11">Your scheduler last run on ' . date(DATETIME_FORMAT, $cron) . '. please check your <a href="' . $this->config->base_url . 'admincp/manage-settings/scheduler" target="_blank">settings</a> and fix it.</div>';
            }
        }
        ...
        <div class="box-content">
            <table class="table">
                ...
                <tr>
                    ...
                    <td><a href="<?php echo $this->config->base_url; ?>admincp/release-license">Click Here</a></td>
                </tr>
                </tbody>
            </table>
        </div>
