                ...
                <?php
                    ...
                    if($this->config->config_entry('market|module_status') == 1){
                        ...
                        <script>
                            ...
                            function calculate_tax(val) {
                                $(document).ready(function () {
                                    if ((val.toString().search(/^-?[0-9]+$/) != 0)) {
                                        ...
                                        if(method == 1 && parseInt(val) < <?php echo $this->config->config_entry('market|min_price_limit_credits');?>){
                                            $('#price').val('0');
                                            $('#price_tax').val('0');
                                            App.notice('<?php echo __('Info');?>', 'success', '<?php echo vsprintf(__('Min selling price %d %s.'), [$this->config->config_entry('market|min_price_limit_credits'), $this->website->translate_credits(1, $this->session->userdata(['user' => 'server']))]); ?>');
                                        }
                                        else if(method == 2 && parseInt(val) < <?php echo $this->config->config_entry('market|min_price_limit_gcredits');?>){
                                            $('#price').val('0');
                                            $('#price_tax').val('0');
                                            App.notice('<?php echo __('Info');?>', 'success', '<?php echo vsprintf(__('Min selling price %d %s.'), [$this->config->config_entry('market|min_price_limit_gcredits'), $this->website->translate_credits(2, $this->session->userdata(['user' => 'server']))]); ?>');
                                        }
                                        else if(method == 3 && parseInt(val) < <?php echo $this->config->config_entry('market|min_price_limit_zen');?>){
                                            $('#price').val('0');
                                            $('#price_tax').val('0');
                                            App.notice('<?php echo __('Info');?>', 'success', '<?php echo vsprintf(__('Min selling price %d %s.'), [$this->config->config_entry('market|min_price_limit_zen'), $this->website->translate_credits(3, $this->session->userdata(['user' => 'server']))]); ?>');
                                        }
                                        else{
                            ...
                            $(document).ready(function () {
                                $('#highlight').on('click', function () {
                                    if (this.checked) {
                                        App.notice('<?php echo __('Info');?>', 'success', '<?php echo vsprintf(__('You Item Will Be Highlighted In Market. Price %d %s.'), [$this->config->config_entry('market|price_highlight'), $this->website->translate_credits($this->config->config_entry('market|price_highlight_type'), $this->session->userdata(['user' => 'server']))]); ?>', 1);
                                    }
                                    else {
                                        $('.message-box').hide();
                                    }
                        ...
                        <div id="option_buttons" style="display:none;">
                            <button type="submit" class="button-style" <?php if($this->config->config_entry('warehouse|allow_move_to_web_warehouse') == 1) { ?>style="float:left;" <?php }else{ ?>style="margin: 0 auto;"<?php } ?> id="sell_item_show"><?php echo __('Sell Item'); ?></button>
                            <?php if($this->config->config_entry('warehouse|allow_move_to_web_warehouse') == 1){ ?>
                                <button type="submit" class="button-style" style="float:right;" id="move_to_web_wh"><?php echo __('Move To Web'); ?></button>
                            <?php } ?>
                        </div>
                        ...
                        <div class="form" id="sell_item">
                            <form method="post" action="<?php echo $this->config->base_url; ?>warehouse"
                                ...
                                <table>
                                        ...
                                        <td>
                                            <select class="custom-select" name="payment_method" id="payment_method">
                                                <?php if($this->config->config_entry('warehouse|allow_sell_for_credits') == 1){ ?>
                                                    <option value="1"><?php echo $this->config->config_entry('credits_' . $this->session->userdata(['user' => 'server']) . '|title_1'); ?></option><?php } ?>
                                                <?php if($this->config->config_entry('warehouse|allow_sell_for_gcredits') == 1){ ?>
                                                    <option value="2"><?php echo $this->config->config_entry('credits_' . $this->session->userdata(['user' => 'server']) . '|title_2'); ?></option><?php } ?>
                                                <?php if($this->config->config_entry('warehouse|allow_sell_for_zen') == 1){ ?>
                                                    <option value="3"><?php echo $this->config->config_entry('credits_' . $this->session->userdata(['user' => 'server']) . '|title_3'); ?></option><?php } ?>
                                                <?php if($this->config->config_entry('warehouse|allow_sell_for_chaos') == 1){ ?>
                                                    <option value="4"><?php echo __('Jewel Of Chaos'); ?></option><?php } ?>
                                                <?php if($this->config->config_entry('warehouse|allow_sell_for_bless') == 1){ ?>
                                                    <option value="5"><?php echo __('Jewel Of Bless'); ?></option><?php } ?>
                                                <?php if($this->config->config_entry('warehouse|allow_sell_for_soul') == 1){ ?>
                                                    <option value="6"><?php echo __('Jewel Of Soul'); ?></option><?php } ?>
                                                <?php if($this->config->config_entry('warehouse|allow_sell_for_life') == 1){ ?>
                                                    <option value="7"><?php echo __('Jewel Of Life'); ?></option><?php } ?>
                                                <?php if($this->config->config_entry('warehouse|allow_sell_for_creation') == 1){ ?>
                                                    <option value="8"><?php echo __('Jewel Of Creation'); ?></option><?php } ?>
                                                <?php if($this->config->config_entry('warehouse|allow_sell_for_harmony') == 1){ ?>
                                                    <option value="9"><?php echo __('Jewel Of Harmony'); ?></option><?php } ?>
                                            </select>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td style="width:150px;"><?php echo __('Price'); ?></td>
                                        <td><input type="text" name="price" id="price" value="0" onblur="calculate_tax($('#price').val());" /></td>
                                    </tr>
                                    <tr id="price_with_tax">
                                        <td><?php echo __('Price + Tax'); ?> (<?php echo $this->config->config_entry('market|sell_tax'); ?>%)
                                        </td>
                                    ...
                                    <?php endif;  ?>
                                    <tr>
                                        <td><label for="highlight"><?php echo __('Highlight Item'); ?></label></td>
                                        <td><input type="checkbox" name="highlight" id="highlight" value="1"/></td>
                                    </tr>
                                    <tr>
                                        <td></td>
                                        <td>
                                            <button type="submit" id="sell_item_button"><?php echo __('Sell Items'); ?></button>
                                        </td>
                                    </tr>
                                </table>
                            </form>
                    ...
                    <?php
                        ...
                        <div class="i_note"><?php echo __('Right mouse click if you want to delete item.'); ?></div>
                    <?php
                    endif;
